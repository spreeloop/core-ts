...
import { fakeFirestoreClientDatabase } from './fake_firestore_client_interface';
...
describe('FirestoreClientDatabase', () => {
  const database = Database.createFirestoreClient(
    fakeFirestoreClientDatabase as unknown as Firestore
  );
  ...
  describe('getCollection', () => {
    it('should run successfully.', async () => {
      ...
      expect(document2.length).toBe(1);
      expect(document.length).toBe(1);
      const docData = document[0].data as { key: string };
      expect(docData.key).toBe('value');
    });
  ...
  describe('findNearestVectorsInCollectionGroup', () => {
    it('should return results for collection group', async () => {
